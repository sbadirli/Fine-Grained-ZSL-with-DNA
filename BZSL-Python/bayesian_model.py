...
class Model(object):
    ...
    def bayesian_cls_train(self, x_tr, y_tr, unseenclasses, att, k_0=0.1, k_1=10, m=5*500, mu_0=0, s=1, Sigma_0=0, K=2, pca_dim=0, tuning=False):
    ...
    def train_and_eval(self):
        ...
        Sig_s,mu_s,v_s,class_id,_=self.bayesian_cls_train(xtrain, ytrain, dataloader.unseenclasses, att, k_0=k_0, k_1=k_1, m=m, \
        ...
        acc_per_cls_s, acc_per_cls_us, gzsl_seen_acc, gzsl_unseen_acc, H = perf_calc_acc(ytest_seen, ytest_unseen, ypred_seen, ypred_unseen)

        print('Results from k0=%.2f, k1=%.2f, m=%d, s=%.1f, K=%d on %s dataset:' % (k_0, k_1, m, s, K, self.dataset))
        print('BSeen acc: %.2f%% Unseen acc: %.2f%%, Harmonic mean: %.2f%%' % (gzsl_seen_acc*100, gzsl_unseen_acc*100, H*100))
